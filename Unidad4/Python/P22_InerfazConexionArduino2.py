...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.segunoplano = QtCore.QTimer()
        self.segunoplano.timeout
    ...
    def accion(self):
        ...
        if texto_boton == "CONECTAR" and self.arduino is None:
            ...
            self.segunoplano.start(100)
            self.btn_accion.setText("DESCONECTAR")
        elif texto_boton == "DESCONECTAR" and self.arduino.isOpen():
            self.segunoplano.stop()
            self.arduino.close()
            self.btn_accion.setText("RECONECTAR")
        else:
            self.arduino.open()
            self.segunoplano.start(100)
            self.btn_accion("DESCONECTAR")

    def lecturaArduino(self):
        if self.arduino is None and self.arduino.isOpen():
